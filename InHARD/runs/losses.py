...
def loss_diversity_hinge_between_two_part(
    part_a, part_b, minthreshold=25
):  # [b, 10, 48, 100]
    ...
    dist = torch.norm(
        part_a[:, :, None, :] - part_b[:, None, :, :], p=2, dim=-1
    )  # b, h, h
    dist = torch.relu(minthreshold - dist)
...
def loss_limb_length_t1(pred, gt, parent_17):
    ...
    limb_gt = torch.norm(gt[:, 1:, :] - gt[:, parent_17[1:], :], dim=2)[
        :, :, None
    ]  # b, 16, 1

    pred = pred.reshape([b, -1, 3, t])  # b, 16, 3, 125
    pred = torch.cat(
        (torch.zeros_like(pred[:, 0:1, :, :]), pred), dim=1
    )  # b, 17, 3, 125
    limb_pred = torch.norm(
        pred[:, 1:, :, :] - pred[:, parent_17[1:], :, :], dim=2
    )  # 沿着 3：[b, 17, 125]
...
def loss_limb_length_t2(pred, gt, parent_17):
    ...
    pred = torch.cat(
        (torch.zeros_like(pred[:, :, 0:1, :, :]), pred), dim=2
    )  # b, 50, 17, 3, 125
    limb_pred = torch.norm(
        pred[:, :, 1:, :, :] - pred[:, :, parent_17[1:], :, :], dim=3
    )  # 沿着 3：[b, 50, 16, 125]
    ...
    for an in ang_names:
        ...
        if lower_bound >= -0.98:
            if torch.any(ang_cos[an] < lower_bound):
                loss += (
                    (ang_cos[an][ang_cos[an] < lower_bound] - lower_bound).pow(2).mean()
                )
        upper_bound = valid_ang[an][1]
        if upper_bound <= 0.98:
            if torch.any(ang_cos[an] > upper_bound):
                loss += (
                    (ang_cos[an][ang_cos[an] > upper_bound] - upper_bound).pow(2).mean()
                )
    return loss
...
def loss_diversity_hinge_v2_likedlow(
    pred, minthreshold=100, scale=50
):  # # [b, 10, 48, 100]
    b, h, vc, t = pred.shape
    triu_indices = np.triu_indices(h, k=1)
    idx = np.arange(h * h).reshape(h, h)[triu_indices]
...
def loss_diversity_hinge_between_two_part_v2_likedlow(
    part_a, part_b, minthreshold=100, scale=50
):  # # [b, 10, 48, 100]
    b, h, vc, t = part_a.shape
    part_a = part_a.reshape(b, h, -1)  # b, 10, 48*100
    part_b = part_b.reshape(b, h, -1)  # b, 10, 48*100
...
def loss_diversity_v2_likedlow_divide(
    outputs, seperate_head, scale=50
):  # b, 50, 42, 60
    b, h, vc, t = outputs.shape
    all_div = []
    for oidx in range(h // seperate_head):
        all_div.append(
            loss_diversity_v2_likedlow(
                outputs[:, oidx * seperate_head : (oidx + 1) * seperate_head, :, :],
                scale=scale,
            )
        )  # [10, 48, 100], [1, 48, 100]
        for ojdx in range(oidx + 1, h // seperate_head):
            all_div.append(
                loss_diversity_two_part_v2_likedlow(
                    outputs[:, oidx * seperate_head : (oidx + 1) * seperate_head, :, :],
                    outputs[:, ojdx * seperate_head : (ojdx + 1) * seperate_head, :, :],
                    scale=scale,
                )
...
def compute_diversity_between_twopart(
    part_a, part_b
):  # [10, 48, 100], [1, 48, 100], [n, 48, 100]
...
def compute_bone_percent_error(gt_pose, motions, parent_17):
    ...
    gt_pose = torch.cat(
        (torch.zeros_like(gt_pose[:, 0:1, :]), gt_pose), dim=1
    )  # 1, 15, 3
    gt_bone = torch.norm(gt_pose[:, 1:, :] - gt_pose[:, parent_17[1:], :], p=2, dim=2)[
        ..., None
    ]  # b, 14, 1

    motions = torch.cat(
        (torch.zeros_like(motions[:, 0:1, :, :]), motions), dim=1
    )  # n, 15, 3, 60
    motions_bone = torch.norm(
        motions[:, 1:, :, :] - motions[:, parent_17[1:], :, :], p=2, dim=2
    )  # n, 14, 60
